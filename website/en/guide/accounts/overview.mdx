...
// Filtered list across mediums
const filtered = await userSession.accounts.list({
  medium: 'card',
...
console.log(`Filtered accounts: ${filtered.accounts.length}`);

// Medium-specific list with the same filters
const activeCards = await userSession.accounts.card.list({
  status: 'active',
  createdAfter: '2026-01-01T00:00:00.000Z',
});
console.log(`Active cards: ${activeCards.accounts.length}`);
```
