...
<QuestHeader
  prereqs={["Read The World → Accounts before this page"]}
  unlocks={["Know which account type to use for your use case"]}
/>
...
:::tip Start with a virtual account
Every account type below draws its balance from a **ledger**, and a
[virtual account](/sdk/guide/accounts/virtual) is what creates one. Create it first — its
`ledgerId` comes back in the create response — then pass that `ledgerId` to every other account you
want to share the balance. See [Ledger & Balance](/sdk/guide/world/ledger) for the concept.
:::
...
[Learn more about BRE-B](/sdk/guide/accounts/breb)

### External US Bank (Plaid)

Link a US bank account using Plaid (via Brale). This is a two-step flow: get a `linkToken`, run Plaid Link on your frontend, then exchange the `publicToken`.

[Learn more about External US Bank](/sdk/guide/accounts/external-us-bank)

### PIX Account Plugin

Plan Brazil PIX account plugin activation:
- **Pix keys**: Supports planning for `CPF`, `CNPJ`, `PHONE`, `EMAIL`, and `EVP` key formats
- **QR initiation**: Supports BR Code / PIX QR planning as a distinct input format
- **Sales-gated activation**: Available only after a sales call

[Learn more about PIX](/sdk/guide/accounts/pix)

### SPEI Account Plugin

Plan Mexico SPEI account plugin activation:
- **CLABE-first**: Plan around the 18-digit CLABE as the primary account identifier
- **Additional identifiers**: SPEI can also reference debit card and linked mobile identifiers
- **Sales-gated activation**: Available only after a sales call

[Learn more about SPEI](/sdk/guide/accounts/spei)
...
[Learn more about Account Webhooks](/sdk/guide/accounts/webhooks)
...
- [Account Webhooks](/sdk/guide/accounts/webhooks) - Real-time account lifecycle notifications
...
- [BRE-B](/sdk/guide/accounts/breb) - Create and resolve BRE-B keys
- [PIX](/sdk/guide/accounts/pix) - Plan Brazil PIX account plugin activation
- [SPEI](/sdk/guide/accounts/spei) - Plan Mexico SPEI account plugin activation
