...
<QuestHeader
  ...
  unlocks={["Virtual account (base account)", "Ledger ID for attaching other accounts"]}
  time="5 min"
/>
...
console.log('Status:', account.status);         // 'creation_in_progress'
console.log('Ledger ID:', account.ledgerId);    // '0xaedf3f21...' — already usable
```

:::tip What you can do straight away
`ledgerId` is populated on the create response, so you can attach cards, Polygon accounts and bank
accounts to this ledger in your very next call — there is nothing to wait for to learn it.

The account itself starts `creation_in_progress` while its ledger registers on-chain. During that
window it **can receive funds but cannot send them**. It becomes `active` when registration lands.
Pass `{ waitLedger: true }` as the second argument if the next thing you do is transfer *out* of
the account:

```typescript
const account = await session.accounts.virtual.create(
  { firstName: 'John', lastName: 'Doe' },
  { waitLedger: true },   // polls every 2s until status is 'active'
);
```
:::
...
interface CreateVirtualAccountParams {
  ...
  ledgerId?: string;       // Optional: attach to an existing ledger (a new one is
                           // minted if omitted, and returned in the response)
...
interface VirtualAccount {
  ...
  ledgerId: string;         // Ledger account address — pass this to other accounts
  ledgerAccountUrn?: string; // The same ledger account under its URN
