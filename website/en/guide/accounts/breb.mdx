...
- create a BRE-B account with a BRE-B key
- associate the account to an existing ledger when `ledgerId` is provided, or create one automatically when it is omitted
...
If you pass `ledgerId`, the BRE-B account is associated with that ledger. If you omit it, the SDK creates a ledger automatically.

When `createKey()` succeeds, `data` contains the created BRE-B account:
