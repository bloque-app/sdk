...
<QuestHeader
  prereqs={["Connected SDK client", "Virtual base account (for ledgerId)"]}
  unlocks={["Virtual card", "PCI-safe details URL", "Card spending controls"]}
  time="5 min"
/>
...
interface CreateCardParams {
  ...
  cardType?: 'VIRTUAL' | 'PHYSICAL'; // Default: 'VIRTUAL'
  cardAddress?: CardMailingAddress;  // Required when cardType is 'PHYSICAL'
  webhookUrl?: string;     // Optional webhook for events
  ledgerId?: string;       // Optional ledger account ID
  defaultAsset?: SupportedAsset; // Primary settlement asset, e.g. "DUSD/6"
...
:::tip Typed `defaultAsset`
`defaultAsset` is the typed, canonical way to set a card's primary settlement asset — it's written to `metadata.default_asset` for you. Prefer it over passing `default_asset` inside a raw `metadata` object.
:::
...
interface CardAccount {
  ...
  statusReason?: CardStatusReason; // Set after a status update that supplied one
  detailsUrl: string | null;      // PCI-compliant details URL, null when blocked
  ownerUrn: string;
  ledgerId: string;
  webhookUrl: string | null;
  metadata?: Record<string, unknown>;
  defaultAsset?: SupportedAsset;  // Read back from metadata.default_asset
  createdAt: string;
  updatedAt: string;
...
Pass `cardType: 'PHYSICAL'` with a mailing address to issue a physical card instead:

```typescript title="create-physical-card.ts"
const card = await userSession.accounts.card.create({
  holderUrn: 'did:bloque:your-origin:user-alias',
...
`cardAddress` is required when `cardType` is `'PHYSICAL'` — the SDK throws before making a request if it's missing.

### Setting a PIN or Reporting a Card Lost/Stolen

`update()` (and the status-change helpers) accept a `statusReason` for auditing, plus a `pin` to set or change the card's PIN:

```typescript title="update-card-status.ts"
// Set/change the PIN
await userSession.accounts.card.update({
  urn: card.urn,
  pin: '1234',
});

// Report lost and disable, with a reason attached
await userSession.accounts.card.updateStatus(card.urn, 'disabled', 'LOST');
```

`statusReason` is one of `'CLIENT_INTERNAL_REASON'`, `'USER_INTERNAL_REASON'`, `'POMELO_INTERNAL_REASON'`, `'PROVIDER_INTERNAL_REASON'`, `'LOST'`, `'STOLEN'`, `'BROKEN'`, or `'UPGRADE'`.
...
### Change the Primary Settlement Asset

`defaultAsset` can be set on its own — `metadata` is optional when you're only changing the asset:
...
console.log('Default asset:', updatedCard.defaultAsset);
```
...
const card = await userSession.accounts.card.create({
  urn: userUrn,
  name: 'Standard Card',
  defaultAsset: 'DUSD/6',             // Primary asset for the card (typed, recommended)
  metadata: {
...
:::info
`defaultAsset` and `metadata.default_asset` write to the same place — `defaultAsset` just gives you type-checking and validates against `SupportedAsset`. If both are set, `defaultAsset` wins.
:::

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `spending_control` | `string` | No | Set to `"default"` (or omit). |
| `default_asset` (or top-level `defaultAsset`) | `string` | No | Primary asset (e.g. `"DUSD/6"`). Defaults to `"DUSD/6"`. Prefer the typed `defaultAsset` param on `create`/`updateMetadata`. |
...
| `default_asset` (or top-level `defaultAsset`) | `string` | No | Primary asset. Defaults to `"DUSD/6"`. |
| `fallback_asset` | `string` | No | Fallback asset when the default is unavailable. |
...
interface CardWebhookPayload {
  ...
  event: CardEventType;
  lifecycle_status?: LifecycleStatus;  // Unambiguous lifecycle stage — see below
  transaction_type?: TransactionType;  // Raw transaction type backing lifecycle_status
...
| `cashback_surcharge` | `authorization` | `debit` | Cashback surcharge applied, sent alongside `purchase`. |
...
`purchase` events always carry `type: 'authorization'` — a temporary hold (e.g. a ride-share pre-auth) and a merchant's immediate final charge otherwise look identical on the wire. `lifecycle_status` and `transaction_type` disambiguate what stage of the transaction lifecycle each event represents, so you can safely decide whether to hold, capture, or release funds on your side:

| `type` | `transaction_type` | `lifecycle_status` | Meaning |
|--------|--------------------|--------------------|---------|
| `authorization` | any | `pending_authorization` | Funds held; not yet settled. |
| `adjustment` | `PURCHASE`, `WITHDRAWAL`, `EXTRACASH`, `BALANCE_INQUIRY`, `PAYMENT` | `captured` | Final capture/settlement — funds move for good. |
| `adjustment` | `REFUND` | `refunded` | Money returned on a previously settled charge. |
| `adjustment` | `REVERSAL_PURCHASE`, `REVERSAL_WITHDRAWAL`, `REVERSAL_EXTRACASH`, `REVERSAL_BALANCE_INQUIRY` | `authorization_reversed` | Hold cancelled/released before capture — no funds ever moved. |
| `adjustment` | `REVERSAL_PAYMENT` | `payment_reversed` | A completed payment was reversed. |
| `adjustment` | `REVERSAL_REFUND` | `refund_reversed` | A previously issued refund was itself reversed. |

:::warning Not present on rejections
`lifecycle_status` and `transaction_type` are only sent on `purchase`, `cashback_surcharge`, `credit_adjustment`, and `debit_adjustment` events — rejection events (`rejected_*`) do not include them.
:::
...
### Hold Cancelled Event Example

Also delivered as `credit_adjustment` — but `transaction_type: "REVERSAL_PURCHASE"` and `lifecycle_status: "authorization_reversed"` make clear this is a cancelled/released hold, not a refund of settled funds. No fees are returned because none were ever charged:
...
:::warning Idempotency
Webhook delivery uses idempotency keys based on `${type}-${event}-${transaction_id}`. Your handler should be idempotent — processing the same event twice must produce the same result.
:::
...
console.log('Provisioning data:', result.activationData);
console.log('Encrypted pass:', result.encryptedPassData);
```
...
|--------|-----------|------|----------|-------------|
| `tokenizeApple` | `nonce` | `string` | Yes | Device nonce from Apple Pay |
| | `nonceSignature` | `string` | Yes | Signature for the nonce |
| | `certificates` | `string[]` | Yes | Leaf and intermediate certificates |
| `tokenizeGoogle` | `deviceId` | `string` | Yes | Google device identifier |
| | `deviceType` | `string` | Yes | Device type (e.g. `MOBILE_PHONE`) |
| | `provisioningAppVersion` | `string` | Yes | App version |
| | `walletAccountId` | `string` | Yes | Google Wallet account ID |

## Updating a Card

Use the `update` method to modify a card's webhook URL, linked ledger, or metadata:
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `urn` | `string` | Yes | Card account URN |
| `metadata` | `Record<string, unknown>` | No | Metadata to merge |
| `status` | `string` | No | New status |
| `webhookUrl` | `string` | No | Webhook URL for card events |
| `ledgerId` | `string` | No | Ledger account ID to link |

## Cashback Programs

Three approaches to automatic savings on card transactions. All modes work with both default and smart spending controls.

### Mode 1: Interchange Share

Route a portion of interchange revenue to a user's savings pocket using the existing `spending_fees` system:
    ...
    spending_fees: [
      {
        ...
        type: 'percentage',
        value: 0.002,           // 0.2% of every purchase → savings
        category: 'custom',
      },
    ],
  },
...
Charge an extra percentage or flat amount per transaction and route it to a savings pocket:
    ...
    cashback_programs: [
      {
        ...
        fee_type: 'percentage',
        value: 0.05,            // 5% surcharge → savings
      },
    ],
  },
});
...
For flat amounts, set `fee_type: 'flat'` and `value` to the fixed amount in local currency (e.g., `1.0` for $1 per transaction).

### Mode 3: Round Up

Round every purchase up to the next whole unit. A $34.70 purchase becomes $35.00 — the extra $0.30 goes to the target pocket:
...
interface CashbackProgram {
  ...
  fee_type?: 'percentage' | 'flat';   // Required for extra_savings
  value?: number;                      // Rate (0.05 = 5%) or fixed amount
}
```
...
1. The `CashbackDecorator` calculates surcharges from each configured program
2. The transaction amount is inflated (original + surcharges) for the balance check
3. `fee_basis` is set to the original amount — interchange and FX fees are computed on the original, not the inflated total
4. The base spending control (default or smart) debits the full inflated amount
5. Surcharge movements route funds from the treasury to the target pockets
6. A `cashback_surcharge` webhook event reports the breakdown

### Cashback Webhook Event

When cashback programs are active, a `cashback_surcharge` event is delivered alongside the standard `purchase` event:
...
Amounts are in the merchant's local currency. Surcharges do not appear in `fee_breakdown`.

### Cashback Refunds

On credit adjustments (refunds), cashback surcharges are reversed proportionally. A 50% refund returns 50% of each program's surcharge from the savings pockets. No separate webhook event is sent for cashback reversals.
