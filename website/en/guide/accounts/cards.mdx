...
interface CardWebhookPayload {
  ...
  required_usd?: number;       // @deprecated alias of local_amount (rejection events)
}
```
...
A rejected purchase carries the same `merchant`, `local_amount`, `local_currency`, and `medium` context as a successful `purchase` event (but no `fee_breakdown`, since no fees were charged), so you can identify the declined transaction and notify the cardholder. `required_usd` is a deprecated alias of `local_amount`.
...
app.post('/webhooks/cards', express.json(), (req, res) => {
  ...
  switch (payload.event) {
    ...
    case 'rejected_insufficient_funds':
      ...
      console.log(`Attempted: ${payload.local_amount} ${payload.local_currency} at ${payload.merchant?.name}`);
