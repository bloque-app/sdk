...
The CLI installs an [MCP server](https://modelcontextprotocol.io/) that gives your agent access to 38 financial tools — organized in two tiers:

- **Workflows** (12 tools) — high-level operations like "create a card" or "show my finances." One call does everything.
...
**API key mode** (for backend use):

```bash
npx @bloque/cli login --api-key sk_live_... --origin your-origin --alias user-alias
```
...
| `create_card` | Full card setup with optional restrictions and funding |
