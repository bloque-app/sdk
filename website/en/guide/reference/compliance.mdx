...
// 2. Fetch the document to render, plus an activation challenge if this
...
// 3b. With activation — run WebAuthn against init.passkey first
if (init.passkey) {
  const credential = await navigator.credentials.create({
    publicKey: {
      challenge: base64UrlDecode(init.passkey.challenge),
      rp: { id: init.passkey.rpId },
      user: {
        id: base64UrlDecode(init.passkey.userId),
        name: init.passkey.userName,
        displayName: init.passkey.userName,
      },
...
| `tosGate.init({ token })` | `TosGateInitResult` | Fetches the active TOS document and a single-use `csrfToken`. `passkey` is non-null only if this document requires account activation. |
