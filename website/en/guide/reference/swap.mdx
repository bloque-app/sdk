...
| --- | --- | --- |
| `pse` | `fromMediums` (cash-in only) | Colombian real-time bank transfer (23+ banks) |
| `kusama` | `fromMediums` or `toMediums` | Kusama blockchain network |
| `base` | `fromMediums` or `toMediums` | Base network (USDC) |
| `breb` | `fromMediums` or `toMediums` | BRE-B instant payment key |
| `rtp` | `toMediums` (cash-out only) | US instant bank payout via RTP |
| `external-us-bank` | `fromMediums` (cash-in only) | US bank ACH on-ramp |
| `bankTransfer` | `toMediums` (cash-out only) | Any Colombian bank (29 banks) |
| `bancolombia` | `toMediums` (cash-out only) | Bancolombia bank account |
...
const result = await userSession.swap.pse.create({
  ...
  args: {
    ...
    // Required for every PSE payment — the order is rejected up front
    // without it, regardless of the underlying bank gateway.
...
| `args`               | `PsePaymentArgs`     | Yes        | PSE arguments for auto-execution                               |
...
interface PsePaymentArgs {
  ...
  redirectUrl: string;      // Required — bank redirects here once the flow completes
}
```
...
| `amountSrc` | `string` | Conditional | Source amount (required if `type` is `'src'`) |
| `depositInformation.urn` | `string` | Yes | Destination account URN to credit on Kusama |
...
| Field | First pause | After partial payment |
| --- | --- | --- |
| `depositStatus` | `'awaiting'` | `'partial'` |
| `receivedAmount` | `'0'` | scaled COP received so far |
| `remainingAmount` | same as `expectedAmount` | scaled COP still required |
| `keyValue` | one-time key | **unchanged** — send top-ups to the same key |
...
| --- | --- | --- | --- |
| `rateSig` | `string` | Yes | Rate signature from `findRates` |
| `amountSrc` | `string` | Conditional | Source DUSD amount (required if `type` is `'src'`) |
| `depositInformation.owner` | `string` | Yes | Account holder name |
| `depositInformation.accountNumber` | `string` | Yes | Bank account number |
| `depositInformation.routingNumber` | `string` | Yes | ABA routing number |
| `depositInformation.accountType` | `'checking' \| 'savings'` | Yes | Account type |
| `depositInformation.bankName` | `string` | No | Bank name |
| `args.sourceAccountUrn` | `string` | Yes | Kusama account URN to debit DUSD from |

**Errors:** `E_INVALID_DEPOSIT_INFORMATION` when required bank fields are missing or `accountType` is invalid. `E_RTP_SOURCE_ACCOUNT_REQUIRED` when `args.sourceAccountUrn` is missing.

### RTP from Base (USDC → US Bank)

Cash out USDC already sent to an EVM/Polygon account on Base. Same bank `depositInformation` as Kusama RTP. Pass `fromMedium: 'base'` and `args.txHash` of the incoming USDC transfer.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `fromMedium` | `'base'` | Yes | Selects the Base USDC → RTP graph |
| `args.sourceAccountUrn` | `string` | Yes | EVM/Polygon account URN that received USDC on Base |
| `args.txHash` | `string` | Yes | Transaction hash of the incoming USDC transfer |

**Errors:** `E_RTP_SOURCE_ACCOUNT_REQUIRED` when `args.sourceAccountUrn` is missing. `E_RTP_TX_HASH_REQUIRED` when `args.txHash` is missing. The SDK throws `BloqueConfigError` before the request if `fromMedium` is `'base'` and `txHash` is empty.

## External US Bank On-Ramp (ACH → Kusama)

Pull USD from a linked US bank account via ACH; funds teleport to Kusama as DUSD. `toMedium` defaults to `'kusama'`.
...
| --- | --- | --- | --- |
| `rateSig` | `string` | Yes | Rate signature from `findRates` |
| `amountSrc` | `string` | Conditional | Source USD amount in cents (required if `type` is `'src'`) |
| `toMedium` | `'kusama' \| 'base'` | No | Destination chain. Omit or `'kusama'` for DUSD. |
| `depositInformation.ledgerAccountId` | `string` | Yes (Kusama) | Kusama ledger account id to credit |
| `args.sourceAccountUrn` | `string` | Yes | Linked external US bank account URN to pull from |

**Errors:** `E_INVALID_DEPOSIT_INFORMATION` when `ledgerAccountId` is missing on the Kusama path.

### External US Bank On-Ramp to Base (ACH → USDC)

Same linked bank. Pass `toMedium: 'base'` and a destination 0x. No ledger account is required.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `toMedium` | `'base'` | Yes | Selects the ACH → Base USDC graph |
| `depositInformation.walletAddress` | `string` | Yes | Destination 0x on Base |
| `depositInformation.walletName` | `string` | No | Optional label for the destination wallet |

**Errors:** `E_INVALID_DEPOSIT_INFORMATION` when `walletAddress` is missing. The SDK throws `BloqueConfigError` before the request if `toMedium` is `'base'` and `walletAddress` is empty.

## Next Steps

- [Accounts Guide](/sdk/guide/accounts/overview) - Create and manage accounts for swap operations
- [BRE-B Guide](/sdk/guide/accounts/breb) - BRE-B keys and on-ramp deposits
