...
Tenant customer-service operators assume an origin, mint **org-owned**
read-only keys, and optionally impersonate a user of that origin. This is
a server-side flow.

1. Human `connect`/`register` as themselves (user JWT). No tenant powers.
2. `orgs.assumeOrigin(namespace)` → 15-minute `kind: origin-operator` JWT.
...
3. `identity.apiKeys.create` with that session mints a bound `sk_`
   (`bound_origin` set). User JWTs cannot mint bound keys.
4. `identity.apiKeys.exchange`:
...
Ops bind is service-only (`PUT /orgs/{org_urn}/controlled-origins/{namespace}`)
and is not wrapped by the SDK.
