...
async function handleFailedTransfer(queueId: string) {
  ...
  if (details.error.includes('insufficient funds')) {
    ...
    if (!source || !dest) {
      throw new Error('La cuenta origen o destino no existe.');
    }
  }
  
  // Para otros errores, sugerir contactar soporte
  throw new Error(`Error de transferencia: ${details.error}`);
}
```
