...
interface PsePaymentArgs {
  bank_code: string;           // Código del banco (de pse.banks())
  user_type?: 0 | 1;          // 0 para persona natural, 1 para persona jurídica
  customer_email?: string;      // Correo electrónico del cliente
  user_legal_id_type?: string;  // Tipo de identificación legal (ej: 'CC', 'NIT', 'CE')
  user_legal_id?: string;       // Número de identificación legal
...
async function realizarSwapPSE() {
  ...
  const result = await userSession.swap.pse.create({
    ...
    args: {
      bank_code: banks[0].code, // Banco seleccionado por el usuario
...
const rates = await userSession.swap.findRates({
  ...
  amountSrc: '100000000', // 100.000000000 KSM (12 decimales)
});
