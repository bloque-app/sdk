...
Plugins de cuenta próximos / con activación por ventas:
- PIX y SPEI están documentados para planeación de formatos y preparación de activación.
- Todavía no exponen clientes públicos `accounts.pix` o `accounts.spei` en el SDK.
- La activación requiere una llamada con ventas en [calendly.com/hi-bloque/15-min](https://calendly.com/hi-bloque/15-min).
...
`register(alias, origin, params)` soporta:
- `type: 'individual'` con **perfil básico** (al menos `email` o `phone`) o perfil completo.
- `type: 'business'` con campos KYB (`name`, `type`, `legalName`, `taxId`, `incorporationDate`, `country`, `state`, `addressLine1`, `city`, `postalCode`, etc.).
