...
<QuestHeader
  ...
  unlocks={["Cuenta virtual (cuenta base)", "Ledger ID para adjuntar otras cuentas"]}
  time="5 min"
/>
...
console.log('Estado:', account.status);         // 'creation_in_progress'
console.log('ID del Ledger:', account.ledgerId); // '0xaedf3f21...' — ya utilizable
```

:::tip Qué puedes hacer de inmediato
`ledgerId` viene en la respuesta de creación, así que puedes adjuntar tarjetas, cuentas Polygon y
cuentas bancarias a este ledger en tu siguiente llamada — no hay nada que esperar para conocerlo.

La cuenta misma arranca en `creation_in_progress` mientras su ledger se registra on-chain. Durante
esa ventana **puede recibir fondos pero no enviarlos**. Pasa a `active` cuando el registro
aterriza. Pasa `{ waitLedger: true }` como segundo argumento si lo siguiente que harás es
transferir *desde* la cuenta:

```typescript
const account = await session.accounts.virtual.create(
  { firstName: 'Juan', lastName: 'Pérez' },
  { waitLedger: true },   // consulta cada 2s hasta que el status sea 'active'
);
```
:::
...
interface CreateVirtualAccountParams {
  ...
  ledgerId?: string;       // Opcional: adjuntar a un ledger existente (si se omite se
                           // acuña uno nuevo y viene en la respuesta)
...
interface VirtualAccount {
  ...
  ledgerId: string;         // Dirección de la cuenta ledger — pásala a otras cuentas
  ledgerAccountUrn?: string; // La misma cuenta ledger bajo su URN
