...
interface CardWebhookPayload {
  ...
  required_usd?: number;       // @deprecated alias de local_amount (eventos de rechazo)
}
```
...
| `rejected_insufficient_funds` | `authorization` | `debit` | Compra rechazada — saldo insuficiente. Incluye `merchant`, `local_amount` y `local_currency`. |
...
Una compra rechazada incluye el mismo contexto de `merchant`, `local_amount`, `local_currency` y `medium` que un evento `purchase` exitoso (pero sin `fee_breakdown`, ya que no se cobraron comisiones), para que puedas identificar la transacción rechazada y notificar al tarjetahabiente. `required_usd` es un alias obsoleto de `local_amount`.
...
app.post('/webhooks/cards', express.json(), (req, res) => {
  ...
  switch (payload.event) {
    ...
    case 'rejected_insufficient_funds':
      ...
      console.log(`Intentado: ${payload.local_amount} ${payload.local_currency} en ${payload.merchant?.name}`);
