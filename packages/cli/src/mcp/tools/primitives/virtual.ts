...
export function registerVirtualTools(server: McpServer, clients: BloqueClients) {
  server.tool(
    ...
    "Create a virtual account (also called a 'pocket' or 'ledger'). Virtual accounts hold balances and can be linked to cards and polygon accounts via ledgerId. For creating a card, prefer the 'create_card' workflow which handles the pocket automatically.",
    {
  ...
  server.tool(
    ...
    'List all virtual accounts (pockets) owned by the user. Shows name, URN, ledgerId, status, and balances.',
    {},
    async () => {
