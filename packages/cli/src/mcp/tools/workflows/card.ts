 ...
 */
export function matchDomain(domain: string, allowedWebsites: unknown): boolean {
...
export function registerCardWorkflows(
  server: McpServer,
  clients: BloqueClients,
) {
  server.registerTool(
    'create_card',
    {
      ...
      inputSchema: {
        ...
        websites: z
          ...
          .optional()
          .describe(
            'Domains this card should be used for (e.g. "amazon.com", "netflix.com")',
          ),
    ...
    async ({
      ...
      try {
        mcpDebug('tool create_card invoked', {
          ...
          hasWebsites: (websites?.length ?? 0) > 0,
        });
        ...
        try {
          ...
          // No KYC record — treat as not approved
        }
        if (kyc.status !== 'approved') {
          mcpDebug('create_card early return: KYC not approved', {
          ...
          return {
            content: [
              {
                type: 'text',
                text: 'KYC verification is not approved. Please run verify_identity first.',
              },
            ],
            isError: true,
          };
        ...
        if (accountUrn) {
          ...
          const polygonList = await clients.accounts.list({
            medium: 'polygon',
          } as any);
          ...
          const poly = await clients.accounts.polygon.create({
        ...
        return logAndFormatToolError('create_card', err);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        if (kyc.status !== 'approved') {
          mcpDebug('create_disposable_card early return: KYC not approved', {
            kycStatus: kyc.status ?? null,
          });
          return {
            content: [
        ...
        mcpDebug('create_disposable_card success', {
          cardUrn: card.urn,
          lastFour: card.lastFour,
        });
        return {
        ...
        return logAndFormatToolError('create_disposable_card', err);
      }
    },
  );
    ...
    'resolve_card_for_website',
    {
      ...
      inputSchema: {
        url: z
          .string()
          .describe(
            'URL or domain name to match (e.g. "https://amazon.com/dp/123" or "amazon.com")',
          ),
      },
    },
    async ({ url }) => {
      ...
      if (!domain) {
        return {
          content: [
            { type: 'text', text: 'A URL or domain name is required.' },
          ],
          isError: true,
        };
      }
  ...
  server.registerTool(
    'assign_card_to_website',
    {
      ...
      inputSchema: {
        ...
        websites: z
          .array(z.string())
          .describe(
            'Domains to assign (e.g. ["amazon.com", "aws.amazon.com"])',
          ),
        merge: z
          .boolean()
          .optional()
          .default(true)
          .describe(
            'Merge with existing websites (true) or replace them (false)',
          ),
      },
    },
    async ({ cardUrn, websites, merge }) => {
      ...
      if (normalizedNew.length === 0) {
        return {
          content: [
            { type: 'text', text: 'At least one valid domain is required.' },
          ],
          isError: true,
        };
      }
